...
- **常用应用集成**：预置 Hysteria、Alist、Vaultwarden、X-UI-YG 等常用服务的 NixOS 模块。
...
│   ├── web/              # Web 服务 (Nginx, Alist, Vaultwarden, X-UI)
│   └── default.nix
├── hardware/             # 硬件与网络适配
...
- **`base.update`**: 自动化维护体系。
  - **Git 同步**: 自动从远程仓库拉取最新的 NixOS 配置到本地路径（如 `/etc/nixos`）。
  - **自动升级**: 定时执行 `nixos-rebuild`，支持 Flake 或传统模式，可配置随机延迟。
  - **垃圾回收 (GC)**: 定期清理旧的系统版本并自动执行存储层优化。
...
- **Web 应用**: 提供 Alist、Vaultwarden、X-UI-YG 的一键反代接入，只需指定 `domain` 即可完成部署。
